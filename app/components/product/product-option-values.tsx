    ...
    return (
      <Select.Root
        ...
        onValueChange={(v) => {
          ...
          if (found) {
            if (onOptionChange) {
              onOptionChange(optionName, found.name);
            } else {
...
function OptionValue({
  ...
  /*
   * - When onOptionChange is provided, the variant is managed by the parent
   * component (state-based selection). Render as a button so the parent can
   * merge the clicked option with current selections.
   * - When the variant is a combined listing child product that leads to a different URL,
   * we render it as an anchor tag.
   * - Otherwise, render as a button that JS-navigates to the variant URL so
   * SEO bots don't index these as duplicated links.
   */
